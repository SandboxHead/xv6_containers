...
qinit(void)
{
    for (int i = 0; i < NPROC; ++i)
    {
        msg_queue[i].head = -1;
        msg_queue[i].tail = -1;
        initlock(&msg_queue[i].lock, "QueueLock");
    }
}
...
int
push(int index, void * msg)
{
    struct Queue * q = &msg_queue[index];
    if (q->head == -1)
        q->head = 0;
    ...
    strcpy(q->buffer[q->tail], msg);
    return 0;
}

int
pop(int index, char * msg)
{
    struct Queue * q = &msg_queue[index];
    if (q->head == -1)
        return -1;

    strcpy(msg, q->buffer[q->head]);
    ++(q->head);
    if (q->head == q->tail+1){
    	q->head = -1;
	q->tail = -1;
...
/*****************************************************************************/
int
send_message(int sender_pid, int rec_pid, char * msg)
{
    int index;
    for (index = 0; index < NPROC; index++)
        if (ptable.proc[index].pid == rec_pid)
            break;

    acquire(&msg_queue[index].lock);
    int ret = push(index, msg);
    wakeup1(&ptable.proc[index]);
    release(&msg_queue[index].lock);

    return ret;
}

int
recv_message(char * msg)
{
    struct proc * p = myproc();
    int index;
    for (index = 0; index < NPROC; index++)
    ...
    acquire(&msg_queue[index].lock);
    int ret = pop(index, msg);
    if (ret == -1)
    {
        sleep(p, &msg_queue[index].lock);
